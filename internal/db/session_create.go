...
func (_c *SessionCreate) check() error {
	if _, ok := _c.mutation.TokenHash(); !ok {
		return &ValidationError{Name: "token_hash", err: errors.New(`db: missing required field "Session.token_hash"`)}
	}
	if v, ok := _c.mutation.TokenHash(); ok {
		if err := session.TokenHashValidator(v); err != nil {
			return &ValidationError{Name: "token_hash", err: fmt.Errorf(`db: validator failed for field "Session.token_hash": %w`, err)}
		}
	}
	...
	if _, ok := _c.mutation.LastSeen(); !ok {
		return &ValidationError{Name: "last_seen", err: errors.New(`db: missing required field "Session.last_seen"`)}
	}
