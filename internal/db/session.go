...
type Session struct {
	...
	// SHA-256 of the cookie value (hex). 32 random bytes → 64 hex chars.
	TokenHash string `json:"token_hash,omitempty"`
	// CreatedAt holds the value of the "created_at" field.
	CreatedAt time.Time `json:"created_at,omitempty"`
	// Absolute expiry. Renewed (sliding) on each authenticated request.
	ExpiresAt time.Time `json:"expires_at,omitempty"`
	// Last request time, used to compute sliding expiry.
...
func (*Session) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case session.FieldCreatedAt, session.FieldExpiresAt, session.FieldLastSeen:
