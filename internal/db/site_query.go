...
func (_q *SiteQuery) QueryApp() *AppQuery {
...
func (_q *SiteQuery) WithApp(opts ...func(*AppQuery)) *SiteQuery {
	query := (&AppClient{config: _q.config}).Query()
...
func (_q *SiteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Site, error) {
	...
	if query := _q.withApp; query != nil {
		if err := _q.loadApp(ctx, query, nodes, nil,
			func(n *Site, e *App) { n.Edges.App = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SiteQuery) loadApp(ctx context.Context, query *AppQuery, nodes []*Site, init func(*Site), assign func(*Site, *App)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Site)
	for i := range nodes {
		if nodes[i].app_sites == nil {
			continue
		}
		fk := *nodes[i].app_sites
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "app_sites" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
