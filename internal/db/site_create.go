...
func (_c *SiteCreate) SetUpstream(v string) *SiteCreate {
...
func (_c *SiteCreate) SetNillableAppID(id *int) *SiteCreate {
	if id != nil {
		_c = _c.SetAppID(*id)
	}
	return _c
...
func (_c *SiteCreate) SetApp(v *App) *SiteCreate {
...
func (_c *SiteCreate) check() error {
	if _, ok := _c.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`db: missing required field "Site.created_at"`)}
	}
	if _, ok := _c.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`db: missing required field "Site.updated_at"`)}
	}
