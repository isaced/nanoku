...
func (m *Manager) EnsureCaddyContainer(ctx context.Context, caddyfileHostPath string) error {
	...
	// /etc/caddy/Caddyfile, which causes caddy to fall back to
	// /config/caddy/autosave.json from a prior boot — meaning any
	// site created this session is invisible to caddy. The bug
	// surfaces as "I added a site but it's not in the Caddyfile"
...
// ensureCaddyfileExists creates an empty-but-valid Caddyfile at
// path if the file does not already exist. Called from
// EnsureCaddyContainer so the bind-mount source is guaranteed
// to exist before ContainerCreate. Without this, a fresh
// `make dev` (where no Caddyfile has been written yet) creates
// a caddy container with a bind-mounted source that doesn't
// exist; caddy then starts without a config and falls back to
// /config/caddy/autosave.json from a prior boot, which is the
// "I added a site but caddy serves an old one" bug.
//
// The placeholder content is the same shape as a freshly-rendered
// Caddyfile with zero sites: just the global options block.
// caddy reload from regen will overwrite it on the first site
// change, so this only matters for the first boot after a
// Caddyfile is deleted.
//
// Errors creating the parent directory are non-fatal: they
// mean the operator is using a path we don't manage (e.g.
// /etc/caddy/...), and EnsureCaddyContainer should still try
// to bring up caddy — the operator will discover the missing
// file via caddy's own startup error, which is louder than
// a silent mount failure on our side.
func ensureCaddyfileExists(path string) error {
	...
	placeholder := "# Auto-generated by Nanoku. Do not edit.\n# Regenerated on every Site change.\n\n"
